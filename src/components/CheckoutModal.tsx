...
type ModalProps = { clientSecret: string; amount: number; expiresAt: string; productName: string; onClose: () => void; onSuccess: () => void }

function CheckoutForm({ amount, expiresAt, onClose, onSuccess }: Omit<ModalProps, 'clientSecret' | 'productName'>) {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); if (!stripe || !elements) return; setProcessing(true); setError(null); const result = await stripe.confirmPayment({ elements, confirmParams: { return_url: window.location.href }, redirect: 'if_required' }); if (result.error) { setError(result.error.message || 'Payment could not be completed. Your held stock will be released after Stripe reports the result.'); setProcessing(false) } else if (result.paymentIntent?.status === 'succeeded') onSuccess(); else { setError('Payment needs an additional step. Please follow the prompt and try again.'); setProcessing(false) } }
